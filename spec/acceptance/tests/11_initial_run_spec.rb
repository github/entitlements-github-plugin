...
describe Entitlements do
  ...
  it "has the correct change count" do
    ...
    # for it above.
    expect(@result.stderr).to match(log("INFO", "Successfully applied 27 change\\(s\\)!"))
  end
  ...
  it "no-ops a GitHub team role" do
    ...
    expect(obj.map { |item| item["login"] }.sort).to eq(%w[nebelung cheetoh ojosazules khaomanee blackmanx russianblue ragamuffin mainecoon].sort)
  end

  it "updates a GitHub team role (removal of members)" do
    result = github_http_get("/teams/5/members")
    expect(result.code).to eq("200")
    obj = JSON.parse(result.body)
    expect(obj.map { |item| item["login"] }.sort).to eq(%w[blackmanx])
  end
  ...
  it "updates GitHub organization membership for admins" do
    ...
    expected = Set.new(%w[blackmanx ragamuffin mainecoon])
  ...
  it "updates GitHub organization membership for members" do
    ...
    expect(response.code).to eq("200")
    users = Set.new(JSON.parse(response.body)["users"])
    expected = Set.new(["ojosazules", "cyprus", "donskoy", "chausie", "russianblue", "khaomanee", "cheetoh", "nebelung"])
    expect(users).to eq(expected)
  end
end
