...
export GITHUB_SERVER="github.fake"
...
ping_github_server() {
  ping -c 1 -w 1 "$GITHUB_SERVER" 1>&2 && rc=$? || rc=$?
  return $rc
}
...
begin_fold "Verifying network connectivity to the github container"
COUNTER=0
SUCCESS=0
while [ $COUNTER -lt 3 ]; do
  let COUNTER=COUNTER+1
  if ping_github_server; then
    SUCCESS=1
    break
  fi
  sleep 1
...
if [ "$SUCCESS" -eq 0 ]; then
  echo "" 1>&2
  echo "%%%HIGHLIGHT {danger}%%%" 1>&2
  echo "*** Error: Unable to ping host '$GITHUB_SERVER'" 1>&2
  echo "%%%END HIGHLIGHT%%%" 1>&2
  exit 255 1>&2
fi
